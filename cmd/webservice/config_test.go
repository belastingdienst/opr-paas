...
func Test_formatEndpoint(t *testing.T) {
	...
	require.NotPanics(
		t,
		func() { formatEndpoint("abc.DEF:3000") },
		"Should NOT panic because hostname is valid (sanity check)",
	)
	require.NotPanics(
		t,
		func() { formatEndpoint("abc.DEF") },
		"Should NOT panic because hostname is valid (sanity check)",
	)
	require.NotPanics(
		t,
		func() { formatEndpoint("abc.DEF.nl") },
		"Should NOT panic because hostname is valid (sanity check)",
	)
	...
	require.Panics(
		t,
		func() { formatEndpoint("abc#DEF:3000") },
		"Should panic because hostname contains illegal character (#)",
	)
	assert.Panics(
		t,
		func() { formatEndpoint(".abcDEF:3000") },
		"Should panic because hostname starts with illegal character (.)",
	)
	assert.Panics(
		t,
		func() { formatEndpoint("ab..cDEF:3000") },
		"Should panic because hostname contains double dot character (..)",
	)
	assert.Panics(
		t,
		func() { formatEndpoint("-abcDEF:3000") },
	...
	assert.Panics(
		t,
		func() { formatEndpoint("abc.DEF-:3000") },
		"Should panic because hostname ends with illegal character (-)",
	)
	assert.Panics(t, func() { formatEndpoint("abc.DEF.a:3000") }, "Should panic because hostname TLD too short (<2)")
	assert.Panics(
		t,
		func() { formatEndpoint("abc.DEF.666:3000") },
		"Should panic because hostname TLD contains illegal character (666)",
	)
	assert.NotPanics(
		t,
		func() { formatEndpoint("abc.DEF-ghi.net:3000") },
		"Should NOT panic because hostname contains LEGAL character (-)",
	)
	assert.Panics(
		t,
		func() { formatEndpoint("abc.DEF_ghi.net:3000") },
		"Should panic because hostname contains illegal character (_)",
	)
