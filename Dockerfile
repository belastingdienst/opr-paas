...
RUN sed -i "s|CONTROLLERS_VERSION = .*|PAAS_VERSION = \"$VERSION\"|" internal/version/main.go && \
    CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -v -a -o manager main.go && \
    CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -v -a -o crypttool
