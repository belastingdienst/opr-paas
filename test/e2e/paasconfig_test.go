...
func assertPaasReconciliationResumed(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
	...
	require.NoError(t, createSyncStatus(ctx, cfg, paasConfig, func(conds []metav1.Condition) bool {
		return meta.IsStatusConditionTrue(conds, api.TypeActivePaasConfig)
	}))
	...
	// not incremented. Thus we pass the initial generation (i.e. 0), as `waitForStatus` waits to observe a generation change before
	// consulting the match function.
	require.NoError(t, waitForStatus(ctx, cfg, paas, 0, func(conds []metav1.Condition) bool {
