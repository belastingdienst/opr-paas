			...
			Assess("groupsynclist contains the group query",
				func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
					...
					assert.Equal(
						t,
						groupQuery,
						groupsynclist.Data["groupsynclist.txt"],
						"The groupsynclist includes the group query",
					)
...
func assertGroupQueryCreated(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
	...
	assert.Equal(
		t,
		paas.GroupKey2GroupName(groupWithQueryName),
		group.Name,
...
func assertGroupQueryCreatedAfterUpdate(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
	...
	assert.Equal(
		t,
		paas.GroupKey2GroupName(group2Key),
		group2.Name,
	...
	assert.Equal(
		t,
		"my-ldap-host",
		group2.Labels["openshift.io/ldap.host"],
	...
	assert.Equal(
		t,
		group2Query,
		group2.Annotations["openshift.io/ldap.uid"],
		"The ldap.uid annotation should contain group.query value",
	)
	assert.Equal(
		t,
		"my-ldap-host:13",
		group2.Annotations["openshift.io/ldap.url"],
...
func assertLdapGroupRemovedAfterUpdatingKey(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
	...
	paas.Spec.Groups = api.PaasGroups{
		...
		"updatedSecondLdapGroup": api.PaasGroup{
